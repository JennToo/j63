library ieee;
  use ieee.std_logic_1164.all;

-- Pipelined Wishbone target adapter for SRAM
--
-- These wishbone signals are omitted as they are not relevant:
--   stall_o
--   err_o
--   rty_o
--   lock_i
--   tag related signals

entity wb_sram is
  generic (
    addr_width : natural;
    data_width : natural
  );
  port (
    clk_i  : in    std_logic;
    rst_ai : in    std_logic;

    -- Wishbone interface
    wb_dat_i   : in    std_logic_vector(data_width - 1 downto 0);
    wb_dat_o   : in    std_logic_vector(data_width - 1 downto 0);
    wb_ack_o   : out   std_logic;
    wb_addr_i  : in    std_logic_vector(addr_width - 1 downto 0);
    wb_stall_o : out   std_logic;
    wb_sel_i   : in    std_logic_vector((data_width / 8) - 1 downto 0);
    wb_stb_i   : in    std_logic;
    wb_we_i    : in    std_logic;

    -- SRAM interface
    sram_addr_o : out   std_logic_vector(addr_width - 1 downto 0);
    sram_dat_o  : out   std_logic_vector(data_width - 1 downto 0);
    sram_dat_i  : in    std_logic_vector(data_width - 1 downto 0);
    sram_sel_o  : out   std_logic_vector((data_width / 8) - 1 downto 0);
    sram_we_o   : out   std_logic
  );
end entity wb_sram;
